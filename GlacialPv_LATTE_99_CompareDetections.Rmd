...
knit: (function(inputFile, encoding) { 
  rmarkdown::render(
    inputFile, 
    encoding = encoding, 
    output_file = file.path(dirname(inputFile), 'LATTE_Evaluation_20221107.pdf'))
  })
---
...
mnT <- read.csv(file_ir2rgb_manual_C, skip = 2, header = FALSE, stringsAsFactors = FALSE, 
  ...
  mutate(id = row_number()) %>%
  mutate(image_name = basename(image_name)) %>%
  filter(detection_type == "harbor_seal" | detection_type == "split_harbor_seal" | detection_type == "harbor_seal_offThermal" | detection_type ==
           "harbor_seal_partial" | detection_type == "off_ir") %>%
  select(id, detection, image_name, bound_left, bound_top, bound_right, bound_bottom, detection_type) %>%
  mutate(average_x = (bound_left + bound_right)/2,
         average_y = (bound_top + bound_bottom)/2) %>%
...
-   At Columbia Glacier:
    ...
    -   The manual review detected approximately 87% of harbor seals detected at the site (missed (n=67 harbor seals).

    -   Approximately 8.5% (n=43) harbor seals were detected by both manual review methods and *missed* by both model review methods.
...
-   At Disenchantment Bay:
    ...
    -   The manual RGB review detected 98% of harbor seals detected at the site.

    -   A single additional harbor seal with the inclusion of the thermal imagery.
...
-   **Areas in color and thermal images where there is not overlap**: For a couple of reasons, the area captured in the color and thermal images are not completely aligned (because of differences in size of area covered). Possible ways to handle this circumstance:
...
-   Data processing steps (SKH):
    -   Must be done before counting can start:
        ...
        -   Reorganize on the LAN and reimport 2020 data (so that the image lists reflect the corrected network paths) (\~12 hours...the import takes some time)
    ...
    -   Can be done after counting has started:
        ...
        -   Develop shell code (or process using post-processing script) to generate output of harbor seal locations within habitat (with CLC) (unsure time)
