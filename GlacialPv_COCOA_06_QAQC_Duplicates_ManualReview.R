...
RPostgreSQL::dbSendQuery(
  con,
  "UPDATE surv_pv_gla.geo_detections SET qaqc_duplicate = \'dupe_manual_review\' 
                         WHERE detection_id IN (SELECT detection_id FROM surv_pv_gla.tbl_detections_cocoa_manual_duplicates)"
)

RPostgreSQL::dbSendQuery(
  con,
  "UPDATE surv_pv_gla.tbl_detections_processed_rgb SET qaqc_duplicate = \'dupe_manual_review\' 
                         WHERE detection_id IN (SELECT detection_id FROM surv_pv_gla.tbl_detections_cocoa_manual_duplicates)"
)
