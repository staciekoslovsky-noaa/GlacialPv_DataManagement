...
counts <- RPostgreSQL::dbGetQuery(
  con,
  paste0(
    "SELECT * FROM surv_pv_gla.summ_count_cocoa_4tbl_count WHERE project_id = \'",
    project_id,
    "\'"
  )
)
...
RPostgreSQL::dbSendQuery(
  con,
  paste0(
    "DELETE FROM surv_pv_gla.tbl_counts c USING surv_pv_gla.tbl_event e WHERE e.id = c.event_id AND count_type_lku = \'A\' AND survey_id LIKE \'%",
    survey_year,
    "%\'"
  )
)
...
counts <- counts %>%
  mutate(
    id = (1:nrow(counts) + id$max),
    count_type_lku = "A",
    std_error = as.numeric(NA),
    count_comments = paste0(
      "Count auto-populated based on tbl_detections on ",
      Sys.Date()
    )
  ) %>%
...
RPostgreSQL::dbSendQuery(
  con,
  paste0("ALTER SEQUENCE surv_pv_gla.tbl_counts_id_seq RESTART WITH ", new_max)
)
